...
const AttendancePage: React.FC = () => {
  ...
  // Tambahan: Handler untuk update jam pulang
  const updateStudentTimeOut = async (studentId: string) => {
    try {
      setError(null);
      const currentTime = new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' });

      // Panggil API untuk mencatat jam pulang
      await api.put(`/attendance/${studentId}/timeout`, {
        date: selectedDate,
        timeOut: currentTime
      });
      ...
      console.log(`Jam pulang ${studentId} dicatat: ${currentTime}`);
    } catch (err) {
      console.error('Error updating time out:', err);
      let msg = 'Gagal mencatat jam pulang.';
      if (axios.isAxiosError(err) && err.response && err.response.data && err.response.data.message) {
        msg = `Gagal: ${err.response.data.message}`;
      }
      setError(msg);
                      ...
                      filteredStudents.map((student, index) => (
                        <tr key={student.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {student.timeOut ? (
                              <div className="flex items-center">
                                <span>{student.timeOut}</span>
                                {/* Removed unused isAutoTimeOut calculation */}
                              </div>
                            ) : '-'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {(() => {
                              if (student.status === 'present') {
                                let keterangan = student.timeIn && student.timeIn > '07:30:00' ? '⏰ Terlambat' : '✅ Hadir Tepat Waktu';
                                
                                // Tambahkan info auto time out jika ada
                                if (student.timeOut) {
                                  const timeOutHour = parseInt(student.timeOut.split(':')[0]);
                                  const timeOutMinute = parseInt(student.timeOut.split(':')[1]);
                          ...
                          {hasPermission('mark_attendance') && (
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                              <div className="flex justify-end space-x-2">
                                ...
                                {/* Tombol Pulang */}
                                <button 
                                  onClick={() => updateStudentTimeOut(student.id)}
                                  className={`px-2 py-1 text-xs rounded border ${
                                    student.timeOut 
                                      ? 'bg-green-100 text-green-700 border-green-300' 
                                      : 'bg-gray-100 text-gray-700 border-gray-300 hover:bg-green-50 hover:text-green-700 hover:border-green-400'
                                  }`}
                                  disabled={!!student.timeOut} // disable jika sudah pulang
                                  title={
                                    student.timeOut 
                                      ? (() => {
                                          ...
                                          return isAutoTimeOut 
                                            ? `Otomatis pulang pada ${student.timeOut}` 
                                            : `Sudah pulang pada ${student.timeOut}`;
                                        })()
                                      : 'Pulang'
                                  }
                                >
